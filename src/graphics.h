...
public:
    ...
    void drawWiFiSetupScreen(String ssidText = "", String passwordText = "", int activeField = 0) {
        clearScreen();
        
        tft.setTextSize(2);
        tft.setTextColor(COLOR_CYAN, COLOR_BG);
        tft.drawCentreString("WiFi Setup", 160, 20, 2);
        
        tft.setTextSize(1);
        tft.setTextColor(COLOR_WHITE, COLOR_BG);
        tft.drawString("Network (SSID):", 20, 60, 2);
        
        uint16_t boxColor = (activeField == 0) ? COLOR_LIGHT_BLUE : COLOR_GRAY;
        tft.drawRect(20, 80, 280, 30, boxColor);
        if (activeField == 0) {
            tft.fillRect(20, 80, 280, 30, COLOR_DARK_GRAY);
        }
        tft.setTextColor(COLOR_WHITE, COLOR_BG);
        tft.drawString(ssidText.substring(0, 30), 25, 87, 2);
        
        tft.setTextColor(COLOR_WHITE, COLOR_BG);
        tft.drawString("Password:", 20, 130, 2);
        
        boxColor = (activeField == 1) ? COLOR_LIGHT_BLUE : COLOR_GRAY;
        tft.drawRect(20, 150, 280, 30, boxColor);
        if (activeField == 1) {
            tft.fillRect(20, 150, 280, 30, COLOR_DARK_GRAY);
        }
        ...
        tft.setTextColor(COLOR_WHITE, COLOR_BG);
        tft.drawString(dots.substring(0, 30), 25, 157, 2);
        
        tft.setTextSize(1);
        tft.setTextColor(COLOR_GRAY, COLOR_BG);
        tft.drawCentreString("Tap fields to edit | Swipe right when done", 160, 215, 1);
    }
